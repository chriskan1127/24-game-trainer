...
- Score is displayed as +10 even if players answer quickly enough to gain bonus points from time. The green value shows +10 no matter what, we want it to show what they actually scored. 
...
- When choosing a solution, remember that we want to choose the first solution that does not contain a fraction AND does not contain a negative number. If there is no solution that has neither negative number nor fraction, then choose the first solution without a fraction. If all solutions have fraction, choose solution without negative numbers. 
...
- Major debug: If a player is in the middle of a choosing a solution when their time runs out, their game ends. For example, when clicking on a button and the timer runs out, the following error is displayed: 
 ...
 This needs to be prevented. Think about ways to solve this issue that do not involve hard coding, but instead systematically solve this issue. This solution could be tied to the solution of freezing all buttons when a player succesfully submits answer. 
